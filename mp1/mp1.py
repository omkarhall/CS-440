...
def create_linear_data(num_samples, slope, intercept, x_range=[-1.0, 1.0], noise=0.1):
    x = (x_range[1] - x_range[0]) * np.random.rand(num_samples, 1) + x_range[0]
    y = slope * x + intercept + (2 * noise * np.random.rand(num_samples, 1) - noise)
    return x,y
...
def mse_loss(y_pred, y_true):
    return 1/y_pred.shape[0] * np.dot((y_pred - y_true).T, (y_pred - y_true))
...
def stochastic_gradient_descent(get_batch_gradient, A_init, learning_rate, num_epochs, data_size, batch_size):
    for _ in range(num_epochs):
        rand_i = np.random.permutation(data_size)
        for i in range(0, data_size, batch_size):
            A_init = A_init - learning_rate * get_batch_gradient(A_init, rand_i[i:i + batch_size])
    return A_init
...
def sample_near(num_samples, config, epsilon=0.1):
    return config + np.random.uniform(-epsilon, epsilon, (num_samples, config.shape[0])) 
...
def estimate_ik_gradient(loss, config, num_samples):
    near_samples = sample_near(num_samples, config)
    losses = np.array([loss(i) for i in near_samples])
    maxloss_sample = near_samples[np.argmax(losses)] # sample w/ highest loss
    vector = maxloss_sample - config # vector from config to sample w/ highest loss
    return vector / np.linalg.norm(vector) # unit vector